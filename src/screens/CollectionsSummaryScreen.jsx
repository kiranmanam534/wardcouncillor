...
const CollectionsSummaryScreen = () => {
  ...
  const years = Array.from({length: currentYear - startYear + 1}, (v, i) => ({
  ...
  const getWardwiseCollections = async (year1, month) => {
    ...
    try {
      ...
      console.log(
        `${apiUrl}/api/Collection/get-ward-wise-monthly-collections?Year=${year1}&Month=${month}`,
      );
  ...
  const calculateTotal = () => {
    return WardCollectionsData?.reduce(
  ...
  const renderItem = ({item}) => (
    <DataTable.Row style={styles.infoRow}>
      ...
      <DataTable.Cell>
        {formattedAmount(parseFloat(item?.value), 'en-ZA', 'ZAR', 'currency')}
      </DataTable.Cell>
    </DataTable.Row>
  );
  const Footer = ({total}) => (
    <DataTable.Header style={{backgroundColor: Colors.yellow, marginBottom: 5}}>
      ...
      <DataTable.Title>
        <Text style={{color: Colors.black}}>
          {formattedAmount(parseFloat(total), 'en-ZA', 'ZAR', 'currency')}
        </Text>
      </DataTable.Title>
    </DataTable.Header>
  );
  ...
  return (
    <View style={styles1.container}>
        ...
        <View style={[styles2.container2, {width: '50%'}]}>
          ...
          <View style={styles.inputView}>
            <RNPickerSelect
              ...
              onValueChange={value => {
                handleValueChangeYear(value, 'year');
              }}
      ...
      {statusCode === 200 && !IsSubmitted && WardCollectionsData.length > 0 && (
        <View
          style={[styles.container, {marginVertical: 15, marginHorizontal: 5}]}
          h>
          ...
          <DataTable style={{marginBottom: 10}}>
            <DataTable.Header style={{backgroundColor: Colors.blue}}>
              <DataTable.Title>
                {' '}
                <Text style={{color: Colors.white}}>Ward No</Text>
              </DataTable.Title>
              <DataTable.Title>
